...
<script>
	...
	// Crea una propriet� = true per ogni funzione ammessa dal profilo utente
	...
	if (!CONTEXT.CAN_GRAPH_ALL) // autorizzazioni ai grafici determinate dalla visibilit� sulle agenzie
	{
		<?php 
			...
			// Crea propriet� con nome CONTEXT.CAN_GRAPH_INS ecc.
...
//	    ,logSuccess:true
	...
	Ext.onReady(function() {
    	Ext.QuickTips.init();
      		...
      		?>
		//-------------------------------------------------------------------------------
		// Se l'utente � abilitato a vedere l'eventuale avviso in ingresso, esegue
		...
		// viva la sessione (che scadr� solo quando scade l'intervallo previsto
		...
		DCS.periodicNotify = new Ext.util.DelayedTask ( function() 
		{
			Ext.Ajax.request({
	        		...
	        		success: function(xhr) {
		        		if (xhr.responseText.length>0)  // se c'� qualcosa, � una istruzione js da eseguire
		...
		// Imposta un handler per intercettare (come attivit� utente) tutte le chiamate a funzioni server
		...
		// non c'� bisogno di startare ora il timeout, visto che avverr� comunque
