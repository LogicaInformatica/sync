...
//Define the Grid data and create the Grid
	var caricaDati = function (idPratica,idNota,window,store) {
...
//    });
	...
	*/		defaults:{margins:'0 0 5 0'},
						...
						if(cmbUtenti.getValue()!='Scegliere un destinatario')
						{
							if(checkVoidText())
							{
								frm.submit({
									...
									success: function(){
										win.close();
										//DCS.FormVistaNote.showDetailVistaNote(idPratica,numPraticaN,tipologiaNota,0);
	...
	return {
		showDetailNoteMex: function(idPratica, numPratica, tipoNota, idNota,IdCliente,CodCliente,root,window){
			...
			gridForm = caricaDati(idPratica,idNota,window);
...
<?php
	...
	if($idAzione==288){
 	...
 	trace("Rob ".print_r($idGrid,true));
	$codici = getRow("SELECT CodContratto FROM contratto WHERE IdContratto IN ($ids)");
	
?>
//var store=Ext.getCmp('').getStore();
id = '<?php echo $idGrid?>';
DCS.FormNotaMexPhp.showDetailNoteMex(<?php echo $ids?>,'<?php echo $codici['CodContratto']?>','<?php echo $tipo?>',0,0,'',null);
