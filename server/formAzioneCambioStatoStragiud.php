<?php
// formAzioneCambioStatoStragiud
// Genera la struttura del form di tipo
// "azione cambio stato stragiudiziale"
...
$IdsStati = fetchValuesArray("SELECT DISTINCT IFNULL(IdStatoStragiudiziale,0) FROM contratto WHERE IdContratto IN ($ids)");
if (count($IdsStati)==1) // se assegnato ad un sola stato, nella list box lo deve escludere
{
	...
	if ($esclude == 0)
		$stati = "nessuno";
			...
			$stati = getScalar("SELECT CONCAT(TitoloStatoStragiudiziale) FROM statostragiudiziale WHERE IdStatoStragiudiziale=$esclude");
}
else
{
	$esclude = "0";
...
$comboStati = generaCombo("Nuovo stato","IdStatoStragiudiziale","TitoloStatoStragiudiziale",
		"FROM statostragiudiziale WHERE IdStatoStragiudiziale != $esclude ORDER BY IdStatoStragiudiziale","","true");
