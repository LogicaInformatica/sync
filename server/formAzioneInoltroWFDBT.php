			...
			handler: function() {
					...
					if(arr.length > 0)
					{
					   ...
					   if(Ext.getCmp('dataVendita').getValue()!='') {
		            	...
		            	formPanel.getForm().submit({
							url: 'server/edit_azione.php', method: 'POST',
							params: {idstatoazione: $idstatoazione, idcontratti: '$idcontratti', idAttuatori: Ext.encode(vectAtt), idOldRegolaProvvigione: $IdRegolaProvvigione, txtHTML: document.getElementById('frmPan').innerHTML, valuesHtml: Ext.encode(vectValue), inoltroMultiplo: 'false' },
							success: function (frm,action) {saveSuccess(win,frm,action);},
							failure: saveFailure
						});
					}else{
