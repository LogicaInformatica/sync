			...
			handler: function() {
			   if(formPanel.getForm().isValid()){
				 ...
				 formPanel.getForm().submit({
					 ...
					 params: {idstatoazione: $idstatoazione, idcontratti: '$idcontratti', rifiutoMultiplo: 'true'  },
...
EOT;
	...
	$confermaButton = <<<EOT
	   ...
	   Ext.getCmp('dataVendita').setReadOnly(true);
	   Ext.getCmp('dataVerifica').setReadOnly(true);
	   Ext.getCmp('comboAffido').setReadOnly(true);
	   Ext.getCmp('chkFlag').setDisabled(true);
	   Ext.getCmp('chkFlagIpoteca').setDisabled(true);
	   Ext.getCmp('chkFlagConcorsuale').setDisabled(true);
	   ...
	   formPanel.addButton(
	     {
				...
				handler: function() {
				   ...
				   if (formPanel.getForm().isValid()){
						...
						formPanel.getForm().submit({
							url: 'server/edit_azione.php', method: 'POST',
							params: {idstatoazione: $idstatoazione, idcontratti: '$idcontratti', txtHTML: document.getElementById('frmPan').innerHTML, valuesHtml: Ext.encode(vectValue), rifiutoMultiplo: 'false'}, 
							success: function (frm,action) {saveSuccess(win,frm,action);},
							failure: saveFailure
						});
					}
