...
function doMain()
{
	...
	switch ($task)
	{
		...
		case "sintesiPerLAgenzia": // sintesi di un'agenzia vista dal manager dell'agenzia
			                       ...
			                       // inoltre vede le provv. di tipo "X" e non "C" perch� � limitata dalla data di max visibilit� STR
		...
		case "ricalcolaProvvigione":
			...
			// Se si tratta di riga tipo C, ricalcola anche la riga corrispondente di tipo X (� come la C, ma con visibilit�
...
function saveModifica()
{
	...
	// quindi non arrivano e vengono trattati come Zero, il che � corretto perch� produce uno storno di tutti i 6 importi
	
	//	NOTA BENE: Le spese e gli interessi incassati non sono suddivisibili per rata
	//             vengono per� registrati su ogni rata (nelle views in lettura invece della somma viene fatto il MAX)
