...
function creaFileExperian($where,&$error) {
	...
	if (!$IdExperian) { // ï¿½ la prima volta che si produce questo file 	
	...
	foreach ($rows as $row) {
		...
		$v[] = $StatusPagamenti;
		$v[] = str_pad('',5); //$GiorniSconfino; non popolato perchï¿½ si applica solo ai non rateali? max 90
...
function processExperianFile($IdExperian,$content,&$error) {
	...
	foreach ($items as $item) {
		...
		$v[] = numero(substr($item,132,3)); // mesi dal piï¿½ recente protesto
		$v[] = numero(substr($item,135,3)); // # protesti
		$v[] = numero(substr($item,138,9)); // importo totale protesti
		$v[] = numero(substr($item,147,3)); // mesi dato pubblico piï¿½ recente
		$v[] = numero(substr($item,150,3)); // mesi dato pregiudizievole piï¿½ recente
		...
		$v[] = numero(substr($item,260,3)); // mesi contratto piï¿½ recente con status 0 ultimi 12 mesi
		$v[] = numero(substr($item,263,3)); // mesi contratto piï¿½ recente con status 3-6 ultimi 12 mesi
		$v[] = numero(substr($item,266,3)); // mesi contratto piï¿½ recente in default
