	   ...
	   formPanel.addButton(
          {
			...
			handler: function() {
			   if(formPanel.getForm().isValid()){
				 DCS.showMask();
				 var totDebResiduo = Ext.getCmp('totDebResiduo').getValue();
		         var totImpProposti = Ext.getCmp('totImpProposti').getValue();		
		         formPanel.getForm().submit({
					url: 'server/edit_azione.php', method: 'POST',
					params: {idstatoazione: $idstatoazione, idcontratti: '$idcontratti', totDebResiduo: totDebResiduo, totImpProposti: totImpProposti, autorizMultiplo: 'true'  },
	   ...
	   formPanel.addButton(
	     {
				...
				handler: function() {
				   ...
				   if (formPanel.getForm().isValid()){
						...
						formPanel.getForm().submit({
							url: 'server/edit_azione.php', method: 'POST',
							params: {idstatoazione: $idstatoazione, idcontratti: '$idcontratti', txtHTML: document.getElementById('frmPan').innerHTML, valuesHtml: Ext.encode(vectValue), autorizMultiplo: 'false'}, 
							success: function (frm,action) {saveSuccess(win,frm,action);},
							failure: saveFailure
						});
					}
