...
//----------------------------------------------------------------------------------------------------------------------
// leggiEsito
// Scopo: 		restituisce al chiamante le informazioni sull'esito di un batch di importazione già eseguito
...
//			R\t id \t chiave \t messaggio	errore recuperabile (può fare retry) sul record con chiave data
//
...
// Determina se rispondere W (wait) perché ci sono file ancora da processare
...
if ($conn) {
	...
	$res = mysqli_query($conn,"SELECT I.* FROM importlog I,compagnia C WHERE I.IdCompagnia=C.IdCompagnia AND CodCompagnia='$from'"
		...
		." ORDER BY IdImportLog"); // con questa ORDER BY, il record di tipo U/K è l'ultimo, in quanto l'ultimo ad essere
	...
	while ($row = mysqli_fetch_assoc($res)) 
	{
		...
		{
			echo $rowm["ErrorType"]."\t".$rowm["RecordKey"]."\t".$rowm["Message"]."\n";
			$nrows++;
		}
		mysqli_free_result($resm);
		echo $row["ImportResult"]."\t".$row["FileId"]."\t".$row["Message"]."\n";
		$nrows++;
	}
