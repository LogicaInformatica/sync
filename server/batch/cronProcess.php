	...
	try  
	{
		...
		foreach ($datiCnt as $value) 
		{
			...
			if ($value["Condizione"]>"") // condizione booleana da verificare in SELECT senza FROM
			{
				...
				$doIt = ($res==1); 
				if (!$doIt)
					trace("Automatismo non eseguito perch� la condizione non � soddisfatta",FALSE);
			}
			else
				$doIt = TRUE;
...
/**
 * creaFileLettere
 * Crea un file contenente tutte le lettere predisposte per l'invio massivo. Se sono di tipo txt, il file � semplicemente una concatenazione
 * di files; se sono di tipo pdf, il file � uno zip che le contiene tutte. Invia poi una mail di notifica agli utenti predefiniti
...
// Cancella i files importati andati a buon fine (cartella okFiles) pi� vecchi di un mese
//================================================================================
...
//================================================================================
////// NOTA 13/12/2013: sembra giusti ma d� sempre FALSE. per ora non utilizzato
