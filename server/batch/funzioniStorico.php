...
//==============================================================================================
// Effettua il recupero dal db storico di uno o pi� clienti e di tutte le tabelle collegate.
//
...
//	 4) dir			path della directory contenente uno o pi� file di import del tipo xxx_yyy_contratti
//
//	 default: dir=TMP_PATH."/import
//
// Nota: la funzione esegue anche degli echo perch� richiamata come pagina a se stante, sia
...
//		}
	...
	if (count($clienti)>0) {
		...
		trace("Clienti da aggiungere, se non gi� inclusi: $sql",false);
	...
	*/
	// Crea una traccia sulla storia dei contratti recuperati, solo se gi� dotati di righe di storia
	getUserName($IdUser);
...
//==============================================================================================
// restoreClient
// Effettua il restore di un cliente e di tutte le entit� collegate
// Argomenti:
...
//==============================================================================================
// restoreSimple
// Execute per il trasferimento dei record di una entit� direttamente figlia di Cliente o Contratto
// Argomenti:
...
//==============================================================================================
// restoreIn
// Execute per il trasferimento dei record di una entit� di secondo livello rispetto al Cliente
// o al Contratto. 
// Argomenti:
...
function restoreDirectory($path, &$contratti, &$clienti) {
	...
	try {
		foreach (scandir($path) as $item) { // legge in ordine alfabetico
			$filename = "$path/$item";
			if (is_file($filename)) {
				// Separa tipo e id del file da processare (il nome file � Company_idfile_tipofile)
	...
	try {
		...
		for ($nrows=0; ($buffer = fgets($file)) !== false; $nrows++) {
			...
			if (NULL == $json) {
				...
				if (property_exists($json,"rows")) { // si tratta dell'ultima riga di controllo (gi� controllata nella import.php)
					break;
				}
...
// criteri di non negativit� dei contratti, non affido e cambiamento di stato precedente un 
...
function svecchiamento($mesi) {
	...
	//flush();
		...
		// contratti non storicizzabili perch� cliente referenziato su Experian (non storicizzo Experian)
	...
	// Crea una traccia sulla storia dei contratti svecchiati, solo se gi� dotati di righe di storia
	getUserName($IdUser);
