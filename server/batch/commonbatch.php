...
function writeResult($id,$esito,$msg)
{
	...
	try
	{
		if ($id>0)
		{
			...
			else if ($esito!='K') // se K, gi� tracciato dalla writeProcessLog
				trace($msg,FALSE);
		}
	}
	catch (Exception $e) 
...
//----------------------------------------------------------------
// checkSemaphore
// Controlla che il batch non sia gi� in esecuzione
// Torna FALSE se il job � gi� in esecuzione (cio� se il record di
...
// Controlla che il batch non sia gi� in esecuzione e mette il
// flag per tali controllo
// Torna FALSE se il job � gi� in esecuzione (cio� se il record di
// chiave 1 ha Status=R oppure se ImportResult non � U)
// (Se ImportResult � NULL significa che � ancora la vecchia gestione
...
function checkAndSetSemaphore()
{
	try
	{
		if (rowExistsInTable("importlog","IdImportLog=1 AND (Status='R' OR ImportResult!='U')"))
			return FALSE; // gi� in esecuzione oppure invio files non completo
...
function resetSemaphore()
{
	try
	{
		...
		// Mette P nell'importResult se questo � non nullo, il che indica che la nuova gestione con fineInvio.php � in funzione
...
if (!function_exists('writeProcessLog')) {
	function writeProcessLog($process,$text,$level=0)
	{
		...
		return execute($sql,false); // evita traccia perch� se MySql � KO , va in ricorsione
	}
}