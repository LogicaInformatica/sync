			...
			case "updateDist":
				...
				if($idDistint!="")
				{
					...
					if ($_FILES['docPath']['name'] != "")
					{
						...
						$urlAttuale=''; // caso in cui non c'� il vecchio file su tabella e non c'� il docPath
						//se non c'� il path sono due i casi:
						//1. Vi � il vecchio url da tabella e basta
						//2. Il vecchio url da tabella � stato eliminato logicamente SULLA FORM e l'utente ha 
						...
						//	 Lo notiamo dal fatto che se il bottone elimina � VISIBILE sulla form (bEliminaVis=false)
						...
						if(($_REQUEST['bEliminaVis']=='true') && ($UrlAllegatoVecchio!=ATT_PATH."/")){
							...
							//il bottone � visibile...si usa il vecchio
						}elseif(($_REQUEST['bEliminaVis']=='false') && ($UrlAllegatoVecchio!=ATT_PATH."/"))
							{	//il bottone � invisibile, c'� il docPath ma � vuoto(cancellato da utente)
								$urlAttuale='';
								if(eliminaRicevuta($UrlAllegatoVecchio))
									$UrlAllegatoVecchio='';
							}
