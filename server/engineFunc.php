...
//			writeHistory("NULL","Applicata una precedente richiesta di forzatura",$IdContratto,"");
		...
		// Se gi� affidata, non affida (la pratica resta per il momento all'agenzia che ce l'ha)
		...
		foreach ($regole as $regola)
		{
			...
			if ($IdRepartoScelto>0)// regola provvigione gi� determinata dalle forzature
			{
			...
			if ($regola["GiorniFissiInizio"]>"") // affido possibile solo in giorni prefissati (cio� "per lotti")
			{
				...
				if ($bool!="1") // oggi non � uno dei giorni stabiliti
				{
...
//7/10/13					$condImporti = "DataFineAffido BETWEEN ('$DataLotto'+interval 1 day-interval 1 month) AND '$DataLotto' AND IdAgenzia=$IdAgenzia";
			...
			// viene esaminata tante volte quante regole distinte esistono, perch� l'applicabilit�
			// della regola non si pu� testare direttamente con la query 
			//-------------------------------------------------------------------------------------
			$dati = getFetchArray("SELECT * FROM $view WHERE $condImporti ORDER BY IdContratto");
			if (!is_array($dati))
			{
				rollback();
				return FALSE;
			}	
			trace("Analisi di ".count($dati)." contratti con IdAgenzia=$IdAgenzia e DataFineAffido=$DataLotto",FALSE);
			$numRiconosciuti = 0;
			$lastId = 0;
			foreach ($dati as $riga)
			{
				//-------------------------------------------------------------------------------------
				// Determina quale regola di calcolo provvigione � applicabile a questo contratto
				//-------------------------------------------------------------------------------------				
	...
	//			trace("IdContratto=$IdContratto, IdRegola=$IdRegola, IdRegolaProvv=$IdRegolaProvv",FALSE);
				if ($IdRegola==$IdRegolaProvv) // � proprio quella corrente 
				{
					...
					if ($riga["ImpPagato"]>0)
					{
						...
						// il capitale  incassato tiene conto di tutto quello che � stato incassato, tranne spese e interessi
					...
					if ($regola["FlagPerPratica"]=='Y') // calcolo provvigioni per pratica singola (Nicol Rinegoziazione)
					{
						$ImpProvvigioni = calcolaProvvigionePerPratica($regola,$IdContratto,$DataLotto);
			...
			if ($numAffidati>0)
			{
				...
				//-------------------------------------------------------------------------------------
				// Caso 0 (nuovo): provvigioni calcolate per pratica; il risultato � dato dalla
				...
				else if (!($regola["FormulaFascia"]>"")) // non c'� calcolo per fasce
				{
					if (!($regola["Formula"]>""))
					{
						trace("La regola provvigione con id=$IdProvvigione non ha n� formula n� fascia",FALSE,TRUE);
						$provv = 0;
					}
					else
					{	
				...
				else
				{
					...
					// Eventuale Bonus: se c'� una formula anche nella riga principale
					if ($regola["Formula"]>"")
					{
...
function calcolaProvvigionePerPratica($regola,$IdContratto,$DataLotto)
{
	...
	//-------------------------------------------------------------------------------------
	// Legge provvigione + campi calcolati
	if (!($regola["FormulaFascia"]>"")) // non c'� calcolo per fasce
	{
		if (!($regola["Formula"]>""))
		{
			trace("La regola provvigione con id=$IdProvvigione non ha n� formula n� fascia",FALSE,TRUE);
		...
		{	
			$provv = getScalar("SELECT ".$regola["Formula"]." FROM v_contratto_per_provvigione WHERE IdContratto=$IdContratto");
	...
	{
		$sql = "SELECT ".$regola["FormulaFascia"]." FROM v_contratto_per_provvigione WHERE IdContratto=$IdContratto";
		$valore = getScalar($sql);
		if ($valore===FALSE)
			return FALSE;
			
		if ($valore==NULL) // non c'� (ad es. nuovoTasso non determinabile, perch� la rinegoziazione non � chiusa)
		{
			$provv = 0;		
		}
		else
		{
			...
			// Calcola l'importo con una SELECT della formula dalla view
			$provv  = getScalar("SELECT ".$fascia["Formula"]." FROM v_contratto_per_provvigione WHERE IdContratto=$IdContratto");
		}
	}
	return $provv;
}
