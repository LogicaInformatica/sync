...
function showData() {
	...
	$sql = "SELECT *,m.DataIni as IniVal,m.DataFin as FinVal,m.lastupd as lastMod,m.lastUser as lastU, 
			(case tipomodello when 'E' then 'E-mail' when 'L' then 'Lettera formato testo' when 'S' then 'Sms' when 'X' then 'Sub Modello' 
...
*/
			...
			if($respDec != 'ASCII')
			{
				echo "{success:false, error:\"Non � possibile inserire caratteri accentati o speciali.\"}";
				die();
			}
			$order   = array("\r\n", "\n", "\r");
		...
		if (!preg_match($regexp,$combo))
		{
			...
			if($mod!=''){
				//modifica
				if(!$client)
				{//se non siamo in modalit� protetta poi ch� si sta accedendo da client
					$codMex="MOD_MMODEL";
