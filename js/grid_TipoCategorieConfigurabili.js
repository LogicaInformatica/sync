...
DCS.GridCategoriaTab = Ext.extend(Ext.grid.GridPanel, {
	pagesize: PAGESIZE,
	titlePanel: '',
	btnMenuAzioni: null,
	task: '',
	...
	initComponent : function() { 
		...
		var newRecord = function(btn, pressed)
		{
			...
			myMask.show();
			DCS.showDetailCCAT.create('Nuovo tipo categoria',null,gstore);
			myMask.hide();
	    };
	    ...
	    var delRecord = function(btn, pressed)
	    {
	    	...
	    	if(Arr.length>0){
		    	for(var k=0;k<Arr.length;k++){
		    		if(Arr[k].get('TitoloCategoria')==null || Arr[k].get('TitoloCategoria')=='')
		    			confString += '<br />	- *Titolo assente*';
		    		else
		    			confString += '<br />	-'+Arr[k].get('TitoloCategoria');
		    		vectString = vectString + '|' + Arr[k].get('IdCategoria');
		    	}
		    	Ext.MessageBox.alert('Conferma', "Si desidera eliminare: "+confString+" ?",function(btn, text){
		    	    if (btn == 'ok'){
		    	    	Ext.Ajax.request({
					        ...
					        method: 'POST',
					        params: {task: 'delete',vect: vectString, tipoConf:'categoriaConf'},
					        success: function(obj) {
					            var resp = obj.responseText;
					            console.log("res "+resp);
					            if (resp == '' && vectString!='') {
					                Ext.MessageBox.alert('Esito', 'Le categorie selezionate sono state eliminati.');
					                gstore.reload();
					            } else {
					            	if(resp!=''){
		...
		var fields = [{name: 'IdCategoria', type: 'int'},
							{name: 'TitoloCategoria'},
							{name: 'CodCategoria', type: 'string'},
							{name: 'LastUser'},
							{name: 'LastUpd', type:'date', dateFormat: 'Y-m-d H:i:s'}];

    	var columns = [selM,
    	               	{dataIndex:'IdCategoria',width:10, header:'IdTCat',hidden: true, hideable: false,filterable:true,groupable:false,sortable:false},
    		        	{dataIndex:'TitoloCategoria',	width:130,	header:'Categoria', hideable: true,filterable:true,groupable:false,sortable:true},
    		        	{dataIndex:'CodCategoria',width:50, header:'Codice',align:'center',hidden: false, hideable: false, filterable:true,groupable:false,sortable:true},    		        	
		...
		var gstore = new Ext.data.GroupingStore({
			...
			baseParams:{task: this.task, group: this.groupOn, tipoConf:'categoriaConf'},
			remoteSort: true,
...
//					}
					...
					myMask.show();
					var titolo="";
					if(rec.get('TitoloCategoria')!=null)
						titolo = "Modifica tipo categoria '"+rec.get('TitoloCategoria')+"'";
					else 
						titolo = "Modifica tipo categoria *Titolo assente*";
					DCS.showDetailCCAT.create(titolo,rec,gstore);
					myMask.hide();
				},
				scope: this
			}
		...
		DCS.GridCategoriaTab.superclass.initComponent.call(this, arguments);
		this.activation();
		//this.store.load();
...
DCS.CategoriaConf = function(){
	...
	return {
		create: function(){
			...
			var gridCategoria = new DCS.GridCategoriaTab({
				titlePanel: 'Lista categorie'+subtitle,
...
DCS.dCategoriePanel = Ext.extend(Ext.Panel, {
	recordMod:null,
	titoloProc:'',
	Wmain:'',
	store:null,
...
//									width: 30,
				  ...
				  handler: function() {
	    		 		if (formAggregato.getForm().isValid()){
							...
							formAggregato.getForm().submit({
								...
								params: {task:"saveAgg", tipoConf:'categoriaConf'},
		...
		DCS.dCategoriePanel.superclass.initComponent.call(this);
		//editing
		if(this.recordMod!=null){
			Ext.getCmp(idDin+'id').setValue(this.recordMod.get('IdCategoria'));
			Ext.getCmp(idDin+'nome').setValue(replace_Tospecial_chars(this.recordMod.get('TitoloCategoria')));
			Ext.getCmp(idDin+'codice').setValue(this.recordMod.get('CodCategoria'));
			Ext.getCmp(idDin+'nome').fireEvent('change',Ext.getCmp(idDin+'nome'),this.recordMod.get('TitoloCategoria'),'');
		}
	}	// fine initcomponent
});
Ext.reg('DCS_DettaglioCategoriePanel', DCS.dCategoriePanel);
	
DCS.showDetailCCAT= function(titolo,rec,store)
{
	return {
		create: function(titolo,rec,store){
			...
			wind = new Ext.Window({
				...
				resizable:false,
				items: [{
					xtype: 'DCS_DettaglioCategoriePanel',
					titoloProc:'',
					recordMod:rec,
					store:store
					}]
