/**
 * griglia dei Stati stragiudiziali 
 */
...
DCS.GridStatoStragiudTab = Ext.extend(Ext.grid.GridPanel, {
	pagesize: PAGESIZE,
	titlePanel: '',
	btnMenuAzioni: null,
	task: '',
	...
	initComponent : function() { 
		...
		var newRecord = function(btn, pressed)
		{
			...
			myMask.show();
			DCS.showDetailSSTG.create('Nuovo stato stragiudiziale',null,gstore);
			myMask.hide();
	    };
	    ...
	    var delRecord = function(btn, pressed)
	    {
	    	...
	    	if(Arr.length>0){
		    	for(var k=0;k<Arr.length;k++){
		    		if(Arr[k].get('TitoloStatoStragiudiziale')==null || Arr[k].get('TitoloStatoStragiudiziale')=='')
		    			confString += '<br />	- *Titolo assente*';
		    		else
		    			confString += '<br />	-'+Arr[k].get('TitoloStatoStragiudiziale');
		    		vectString = vectString + '|' + Arr[k].get('IdStatoStragiudiziale');
		    	}
		    	Ext.MessageBox.alert('Conferma', "Si desidera eliminare: "+confString+" ?",function(btn, text){
		    	    if (btn == 'ok'){
		    	    	Ext.Ajax.request({
					        ...
					        method: 'POST',
					        params: {task: 'delete',vect: vectString, tipoConf:'statoStragiudConf'},
					        success: function(obj) {
					            var resp = obj.responseText;
					            console.log("res "+resp);
					            if (resp == '' && vectString!='') {
					                Ext.MessageBox.alert('Esito', 'Gli stati stragiudiziali selezionati sono stati eliminati.');
					                gstore.reload();
					            } else {
					            	if(resp!=''){
		...
		var fields = [{name: 'IdStatoStragiudiziale', type: 'int'},
							{name: 'TitoloStatoStragiudiziale'},
							{name: 'CodStatoStragiudiziale', type: 'string'},
							{name: 'LastUser'},
							{name: 'LastUpd', type:'date', dateFormat: 'Y-m-d H:i:s'}];

    	var columns = [selM,
    	               	{dataIndex:'IdStatoStragiudiziale',width:10, header:'IdSStrad',hidden: true, hideable: false,filterable:true,groupable:false,sortable:false},
    		        	{dataIndex:'TitoloStatoStragiudiziale',	width:130,	header:'Stato Stragiudiziale', hideable: true,filterable:true,groupable:false,sortable:true},
    		        	{dataIndex:'CodStatoStragiudiziale',width:50, header:'Codice',align:'center',hidden: false, hideable: false, filterable:true,groupable:false,sortable:true},    		        	
		...
		var gstore = new Ext.data.GroupingStore({
			...
			baseParams:{task: this.task, group: this.groupOn, tipoConf:'statoStragiudConf'},
			remoteSort: true,
		...
		Ext.apply(this,{
			...
			listeners: {
				celldblclick : function(grid,rowIndex,columnIndex,event){
					...
					myMask.show();
					var titolo="";
					if(rec.get('TitoloStatoStragiudiziale')!=null)
						titolo = "Modifica stato stragiudiziale '"+rec.get('TitoloStatoStragiudiziale')+"'";
					else 
						titolo = "Modifica stato stragiudiziale *Titolo assente*";
					DCS.showDetailSSTG.create(titolo,rec,gstore);// <--- DA FARE
					myMask.hide();
				},
				scope: this
			}
		...
		var tbarItems = [
					...
					'->', {xtype:'button',
							...
							pressed: false,
							enableToggle:false,
							text: 'Nuovo stato stragiudiziale',
							handler: newRecord
							},
					'-', {xtype:'button',
							...
							pressed: false,
							enableToggle:false,
							text: 'Cancella stato stragiudiziale',
							handler: delRecord
							},
	                ...
	                '-', helpButton("StatiStragiudiziali"),' '
				];
		...
		DCS.GridStatoStragiudTab.superclass.initComponent.call(this, arguments);
		this.activation();
		//this.store.load();
...
DCS.StatoStragiudizialeConf = function(){

	return {
		create: function(){
			var subtitle = '<span class="subtit"><br>Gli Stati Stragiudiziali sono attribuibili alle pratiche in gestione legale,'
			...
			return new DCS.GridStatoStragiudTab({
				titlePanel: 'Lista Stati Stragiudiziali'+subtitle,
				flex: 1,
				task: "readMainGrid"
			});
		}
...
DCS.dStatoStragiudPanel = Ext.extend(Ext.Panel, {
	recordMod:null,
	titoloProc:'',
	Wmain:'',
	store:null,
		...
		var formAggregato = new Ext.form.FormPanel({
			...
			items: [{
					...
					items: [{
						...
						items: [{
							fieldLabel: 'IdStatoStragiudiziale',
							readOnly:true,
							hidden:true,
...
//									width: 30,
				  ...
				  handler: function() {
	    		 		if (formAggregato.getForm().isValid()){
							...
							formAggregato.getForm().submit({
								...
								params: {task:"saveAgg", tipoConf:'statoStragiudConf'},
		...
		if(this.recordMod!=null){
			...
			Ext.getCmp(idDin+'nome').setValue(replace_Tospecial_chars(this.recordMod.get('TitoloStatoStragiudiziale')));
			Ext.getCmp(idDin+'codice').setValue(this.recordMod.get('CodStatoStragiudiziale'));
			Ext.getCmp(idDin+'nome').fireEvent('change',Ext.getCmp(idDin+'nome'),this.recordMod.get('TitoloStatoStragiudiziale'),'');
		}
	}	// fine initcomponent
});
Ext.reg('DCS_DettaglioStatoStragiudPanel', DCS.dStatoStragiudPanel);
	
DCS.showDetailSSTG= function(titolo,rec,store)
{
	return {
