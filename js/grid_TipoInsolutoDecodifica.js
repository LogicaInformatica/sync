...
DCS.GridInsolutoTab = Ext.extend(Ext.grid.GridPanel, {
	pagesize: PAGESIZE,
	titlePanel: '',
	btnMenuAzioni: null,
	task: '',
	...
	initComponent : function() { 
		...
		var newRecord = function(btn, pressed)
		{
			...
			myMask.show();
			DCS.showDetailINS.create('Nuovo tipo insoluto',null,gstore);
			myMask.hide();
	    };
	    ...
	    var delRecord = function(btn, pressed)
	    {
	    	...
	    	if(Arr.length>0){
		    	for(var k=0;k<Arr.length;k++){
		    		if(Arr[k].get('TitoloTipoInsoluto')==null || Arr[k].get('TitoloTipoInsoluto')=='')
		    			confString += '<br />	- *Titolo assente*';
		    		else
		    			confString += '<br />	-'+Arr[k].get('TitoloTipoInsoluto');
		    		vectString = vectString + '|' + Arr[k].get('IdTipoInsoluto');
		    	}
		    	Ext.MessageBox.alert('Conferma', "Si desidera eliminare: "+confString+" ?",function(btn, text){
		    	    if (btn == 'ok'){
		    	    	Ext.Ajax.request({
					        ...
					        method: 'POST',
					        params: {task: 'delete',vect: vectString, tipoDec:'insoluto'},
					        success: function(obj) {
					            var resp = obj.responseText;
					            console.log("res "+resp);
					            if (resp == '' && vectString!='') {
					                Ext.MessageBox.alert('Esito', 'I tipi insoluti selezionati sono state eliminati.');
					                gstore.reload();
					            } else {
					            	if(resp!=''){
		...
		var fields = [{name: 'IdTipoInsoluto', type: 'int'},
							{name: 'TitoloTipoInsoluto'},
							{name: 'CodTipoInsolutoLegacy', type: 'string'},
							{name: 'CodTipoInsoluto', type: 'string'},
							{name: 'LastUser'},
							{name: 'LastUpd', type:'date', dateFormat: 'Y-m-d H:i:s'}];

    	var columns = [selM,
    	               	{dataIndex:'IdTipoInsoluto',width:10, header:'IdTS',hidden: true, hideable: false,filterable:true,groupable:false,sortable:false},
    		        	{dataIndex:'TitoloTipoInsoluto',	width:130,	header:'Insoluto', hideable: false,filterable:true,groupable:false,sortable:true},
    		        	{dataIndex:'CodTipoInsolutoLegacy',width:50, header:'Vecchio codice',align:'center',hidden: false, hideable: false, filterable:true,groupable:false,sortable:true},    		        	
    		        	{dataIndex:'CodTipoInsoluto',	width:50,	header:'Nuovo codice',align:'center',hidden: false, hideable: false,filterable:true,groupable:false,sortable:true},
		...
		var gstore = new Ext.data.GroupingStore({
			...
			baseParams:{task: this.task, group: this.groupOn, tipoDec:'insoluto'},
			remoteSort: true,
...
//					}
					...
					myMask.show();
					var titolo="";
					if(rec.get('TitoloTipoInsoluto')!=null)
						titolo = "Modifica tipo insoluto '"+rec.get('TitoloTipoInsoluto')+"'";
					else 
						titolo = "Modifica tipo insoluto *Titolo assente*";
					DCS.showDetailINS.create(titolo,rec,gstore);
					myMask.hide();
				},
				scope: this
			}
		...
		DCS.GridInsolutoTab.superclass.initComponent.call(this, arguments);
		this.activation();
		//this.store.load();
...
DCS.TipoInsoluto = function(){

	return {
		create: function(){
			var subtitle = '<span class="subtit"><br>I codici di "tipo insoluto" marcano i movimenti del partitario che indicano insoluti; si noti in particolare'
				+'<br>il codice 50007, che &egrave; usato dal motore di classificazione per riconoscere i cosiddetti "insoluti tecnici" </span>';

			var gridInsoluto = new DCS.GridInsolutoTab({
				titlePanel: 'Lista tipi di insoluto'+subtitle,
...
DCS.dInsPanel = Ext.extend(Ext.Panel, {
	recordMod:null,
	titoloProc:'',
	Wmain:'',
	invisibileLeg:true,
...
//									width: 30,
				  ...
				  handler: function() {
	    		 		if (formAggregato.getForm().isValid()){
							...
							formAggregato.getForm().submit({
								...
								params: {task:"saveAgg", tipoDec:'insoluto'},
		...
		DCS.dInsPanel.superclass.initComponent.call(this);
		//editing
		if(this.recordMod!=null){
			Ext.getCmp(idDin+'id').setValue(this.recordMod.get('IdTipoInsoluto'));
			Ext.getCmp(idDin+'nome').setValue(replace_Tospecial_chars(this.recordMod.get('TitoloTipoInsoluto')));
			Ext.getCmp(idDin+'codice').setValue(this.recordMod.get('CodTipoInsoluto'));
			Ext.getCmp(idDin+'codiceL').setValue(this.recordMod.get('CodTipoInsolutoLegacy'));
			Ext.getCmp(idDin+'nome').fireEvent('change',Ext.getCmp(idDin+'nome'),this.recordMod.get('TitoloTipoInsoluto'),'');
		}
	}	// fine initcomponent
});
Ext.reg('DCS_DettaglioInsolutoPanel', DCS.dInsPanel);
	
DCS.showDetailINS= function(titolo,rec,store)
{
	return {
		create: function(titolo,rec,store){
			var invisibileLeg=true;
			var h=165;
			if(rec!=null && rec.get('CodTipoInsolutoLegacy')!='' && rec.get('CodTipoInsolutoLegacy')!=null)
			{
				invisibileLeg=false;
				h=190;
			}
			wind = new Ext.Window({
				...
				modal: true,
				title: titolo,
				tools: [helpTool("TipoInsoluto")],
				resizable:false,
				items: [{
					xtype: 'DCS_DettaglioInsolutoPanel',
					titoloProc:'',
					recordMod:rec,
