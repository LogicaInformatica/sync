...
DCS.FormNotaMex = function(){
	...
	var storeRel;
	var kidsN;
// viene reso visibile il panel visibilit� solo agli utenti interni
	VisibilHidden = (CONTEXT.InternoEsterno=='E'); // gli utenti esterni non vedono la sezione destinatario
	// visibilit� flag riservato ed impostazione del valore della checkbox
	// per default il valore � true, ma nel caso di utenti non abilitati
	// imposto false	
	...
	*/		defaults:{margins:'0 0 5 0'},
								...
								if (resp != undefined)
								{
									if(resp.NP!='')
									{
										switch (resp.NP) {
											case '0':
												var isRootSon = rootN.substring(0,1);
												if(isRootSon=='x'){//se � figlio di radice
	...
	return {
		showDetailNoteMex: function(idPratica, numPratica, tipoNota, idNota,IdCliente,CodCliente,
			...
			//controlla se � una risposta di base o se stiamo facendo 
			//una risposta ad un thread gi� popolato con altri operatori.
			Ext.Ajax.request({
				url: 'server/AjaxRequest.php', 
	    		params : {	task: 'read',
