...
DCS.GridTRichiestaTab = Ext.extend(Ext.grid.GridPanel, {
	pagesize: PAGESIZE,
	titlePanel: '',
	btnMenuAzioni: null,
	task: '',
	...
	initComponent : function() { 
		...
		var newRecord = function(btn, pressed)
		{
			...
			myMask.show();
			DCS.showDetailTRIC.create('Nuovo tipo richiesta',null,gstore,true);
			myMask.hide();
	    };
	    ...
	    var delRecord = function(btn, pressed)
	    {
	    	...
	    	if(Arr.length>0){
		    	for(var k=0;k<Arr.length;k++){
		    		if(Arr[k].get('TitoloTipoRichiesta')==null || Arr[k].get('TitoloTipoRichiesta')=='')
		    			confString += '<br />	- *Titolo assente*';
		    		else
		    			confString += '<br />	-'+Arr[k].get('TitoloTipoRichiesta');
		    		vectString = vectString + '|' + Arr[k].get('IdTipoRichiesta');
		    	}
		    	Ext.MessageBox.alert('Conferma', "Si desidera eliminare: "+confString+" ?",function(btn, text){
		    	    if (btn == 'ok'){
		    	    	Ext.Ajax.request({
					        ...
					        method: 'POST',
					        params: {task: 'delete',vect: vectString, tipoConf:'tiporichiestaConf'},
		...
		var fields = [{name: 'IdTipoRichiesta', type: 'int'},
							{name: 'TitoloTipoRichiesta'},
							{name: 'CodTipoRichiesta', type: 'string'},
							{name: 'LastUser'},
							{name: 'Ordine'},
							{name: 'LastUpd', type:'date', dateFormat: 'Y-m-d H:i:s'}];

    	var columns = [selM,
    	               	{dataIndex:'IdTipoRichiesta',width:10, header:'IdTricC',hidden: true, hideable: false,filterable:true,groupable:false,sortable:false},
    		        	{dataIndex:'TitoloTipoRichiesta',	width:130,	header:'Tipo richiesta', hideable: false,filterable:true,groupable:false,sortable:true},
    		        	{dataIndex:'CodTipoRichiesta',	width:50,	header:'Codice',align:'center',hidden: false, hideable: false,filterable:true,groupable:false,sortable:true},
		...
		var gstore = new Ext.data.GroupingStore({
			...
			baseParams:{task: this.task, group: this.groupOn, tipoConf:'tiporichiestaConf'},
			remoteSort: true,
...
//					}
					...
					myMask.show();
					var titolo="";
					if(rec.get('TitoloTipoRichiesta')!=null)
						titolo = "Modifica tipo richiesta '"+rec.get('TitoloTipoRichiesta')+"'";
					else 
						titolo = "Modifica tipo richiesta *Titolo assente*";
					DCS.showDetailTRIC.create(titolo,rec,gstore);
					myMask.hide();
				},
				scope: this
			}
		...
		DCS.GridTRichiestaTab.superclass.initComponent.call(this, arguments);
		this.activation();
		//this.store.load();
...
DCS.TRichiestaConf = function(){

	return {
		create: function(){
			var subtitle = '<span class="subtit"><br>I tipi di richiesta sono liberamente definibili per essere specificati nelle azioni di tipo "registra richiesta del cliente".</span>';
			var gridTrichiesta = new DCS.GridTRichiestaTab({
				titlePanel: 'Lista tipi di richiesta'+subtitle,
...
DCS.recordComboOrdine = Ext.data.Record.create([
              		{name: 'ordine'}]);
DCS.dTRichPanel = Ext.extend(Ext.Panel, {
	recordMod:null,
	titoloProc:'',
	Wmain:'',
	store:null,
	initComponent: function() {
		...
		var idDin = "tiporichiesta";
		var extStore = this.store;
		var nuovo = this.nuovo;
		var queryComboOrdine = "SELECT ordine FROM tiporichiesta group by ordine order by ordine asc;";
...
//									regexText:"Inserire solo valori numerici o lasciare vuoto",
				  ...
				  handler: function() {
	    		 		if (formAggregato.getForm().isValid()){
							...
							formAggregato.getForm().submit({
								...
								params: {task:"saveAgg", tipoConf:'tiporichiestaConf', isNew:nuovo},
		...
		DCS.dTRichPanel.superclass.initComponent.call(this);
		//editing
		ds.load({
//			params:{sql:queryCombo},
			callback : function(rows,options,success) 
			{
				if(this.recordMod!=null){
					Ext.getCmp(idDin+'id').setValue(this.recordMod.get('IdTipoRichiesta'));
					Ext.getCmp(idDin+'nome').setValue(replace_Tospecial_chars(this.recordMod.get('TitoloTipoRichiesta')));
					Ext.getCmp(idDin+'codice').setValue(this.recordMod.get('CodTipoRichiesta'));
					Ext.getCmp(idDin+'cmbOrdine').setValue(this.recordMod.get('Ordine'));
					Ext.getCmp(idDin+'nome').fireEvent('change',Ext.getCmp(idDin+'nome'),this.recordMod.get('TitoloTipoRichiesta'),'');
				}
			},
			scope:this
		});
...
Ext.reg('DCS_DettaglioTipoRichiestaConfPanel', DCS.dTRichPanel);
	
DCS.showDetailTRIC= function()
{
	return {
		create: function(titolo,rec,store,isnew){
			...
			wind = new Ext.Window({
				...
				modal: true,
				title: titolo,
				tools: [helpTool("TipoRichiesta")],
				resizable:false,
				items: [{
					xtype: 'DCS_DettaglioTipoRichiestaConfPanel',
					titoloProc:'',
					recordMod:rec,
					store:store,
					nuovo:isnew
