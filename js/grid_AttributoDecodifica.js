...
DCS.GridAttributoTab = Ext.extend(Ext.grid.GridPanel, {
	pagesize: PAGESIZE,
	titlePanel: '',
	btnMenuAzioni: null,
	task: '',
	...
	initComponent : function() { 
		...
		var newRecord = function(btn, pressed)
		{
			...
			myMask.show();
			DCS.showDetailATT.create('Nuovo attributo',null,gstore);
			myMask.hide();
	    };
	    ...
	    var delRecord = function(btn, pressed)
	    {
	    	...
	    	if(Arr.length>0){
		    	for(var k=0;k<Arr.length;k++){
		    		if(Arr[k].get('TitoloAttributo')==null || Arr[k].get('TitoloAttributo')=='')
		    			confString += '<br />	- *Titolo assente*';
		    		else
		    			confString += '<br />	-'+Arr[k].get('TitoloAttributo');
		    		vectString = vectString + '|' + Arr[k].get('IdAttributo');
		    	}
		    	Ext.MessageBox.alert('Conferma', "Si desidera eliminare: "+confString+" ?",function(btn, text){
		    	    if (btn == 'ok'){
		    	    	Ext.Ajax.request({
					        ...
					        method: 'POST',
					        params: {task: 'delete',vect: vectString, tipoDec:'attributo'},
		...
		var fields = [{name: 'IdAttributo', type: 'int'},
							{name: 'TitoloAttributo'},
							{name: 'CodAttributoLegacy', type: 'string'},
							{name: 'CodAttributo', type: 'string'},
							{name: 'LastUser'},
							{name: 'LastUpd', type:'date', dateFormat: 'Y-m-d H:i:s'}];

    	var columns = [selM,
    	               	{dataIndex:'IdAttributo',width:10, header:'IdTP',hidden: true, hideable: false,filterable:true,groupable:false,sortable:false},
    		        	{dataIndex:'TitoloAttributo',	width:130,	header:'Attributo', hideable: false,filterable:true,groupable:false,sortable:true},
    		        	{dataIndex:'CodAttributoLegacy',width:50, header:'Vecchio codice',align:'center',hidden: false, hideable: false, filterable:true,groupable:false,sortable:true},    		        	
    		        	{dataIndex:'CodAttributo',	width:50,	header:'Nuovo codice',align:'center',hidden: false, hideable: false,filterable:true,groupable:false,sortable:true},
		...
		var gstore = new Ext.data.GroupingStore({
			...
			baseParams:{task: this.task, group: this.groupOn, tipoDec:'attributo'},
			remoteSort: true,
...
//					}
					...
					myMask.show();
					var titolo="";
					if(rec.get('TitoloAttributo')!=null)
						titolo = "Modifica attributo '"+rec.get('TitoloAttributo')+"'";
					else 
						titolo = "Modifica attributo *Titolo assente*";
					DCS.showDetailATT.create(titolo,rec,gstore);
					myMask.hide();
				},
				scope: this
			}
		...
		DCS.GridAttributoTab.superclass.initComponent.call(this, arguments);
		this.activation();
		//this.store.load();
...
DCS.Attributo = function(){

	return {
		create: function(){
			var subtitle = '<span class="subtit"><br>Gli attributi sono caratterizzazioni aggiuntive dei contratti sul sistema centrale.'
				+'<br>L\'elenco deve rispecchiare fedelmente quanto trattato nei flussi di acquisizione dei Contratti.</span>';

			var gridAttributo = new DCS.GridAttributoTab({
				titlePanel: 'Lista attributi'+subtitle,
...
DCS.dAttPanel = Ext.extend(Ext.Panel, {
	recordMod:null,
	titoloProc:'',
	Wmain:'',
	invisibileLeg:true,
...
//									width: 30,
				  ...
				  handler: function() {
	    		 		if (formAggregato.getForm().isValid()){
							...
							formAggregato.getForm().submit({
								...
								params: {task:"saveAgg", tipoDec:'attributo'},
		...
		DCS.dAttPanel.superclass.initComponent.call(this);
		//editing
		if(this.recordMod!=null){
			Ext.getCmp(idDin+'id').setValue(this.recordMod.get('IdAttributo'));
			Ext.getCmp(idDin+'nome').setValue(replace_Tospecial_chars(this.recordMod.get('TitoloAttributo')));
			Ext.getCmp(idDin+'codice').setValue(this.recordMod.get('CodAttributo'));
			Ext.getCmp(idDin+'codiceL').setValue(this.recordMod.get('CodAttributoLegacy'));
			Ext.getCmp(idDin+'nome').fireEvent('change',Ext.getCmp(idDin+'nome'),this.recordMod.get('TitoloAttributo'),'');
		}
	}	// fine initcomponent
});
Ext.reg('DCS_DettaglioAttributoPanel', DCS.dAttPanel);
	
DCS.showDetailATT= function(titolo,rec,store)
{
	return {
		create: function(titolo,rec,store){
			var invisibileLeg=true;
			var h=165;
			if(rec!=null && rec.get('CodAttributoLegacy')!='' && rec.get('CodAttributoLegacy')!=null)
			{
				invisibileLeg=false;
				h=190;
			}
			wind = new Ext.Window({
				...
				resizable:false,
				items: [{
					xtype: 'DCS_DettaglioAttributoPanel',
					titoloProc:'',
					recordMod:rec,
