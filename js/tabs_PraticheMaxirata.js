...
DCS.GridPraticheMaxirata = Ext.extend(DCS.GridPratiche, {
	IdCategoriaMaxirata:'',
...
{dataIndex:'CodCliente',width:70,	header:'Cod.Cliente',hidden:true,hideable:true},
		...
		Ext.apply(this,{
			...
			IdCategoriaMaxirata:this.IdCategoriaMaxirata
	    });
	       ...
	       this.on('render',function(){
				...
				toolBar.insert(8,{
					   ...
					   handler: function(){Ext.ux.Printer.exportXLS(this,1,"Pratiche Maxirata");},
					   scope: this,
		...
		DCS.GridPraticheMaxirata.superclass.initComponent.call(this,arguments);
	}
});
	...
	return {
		...
		create: function(){
			...
			Ext.Ajax.request({
				...
				params: {
					task: 'read',
					sql: "SELECT IdCategoriaMaxirata,CodMaxirata,CategoriaMaxirata FROM categoriamaxirata"
						 +" UNION ALL SELECT 0,'NUL','Senza categoria' order by CategoriaMaxirata"
				},
				method: 'POST',
				autoload: true,
				success: function(result, request){
					...
					for (i = 0; i < resp.total; i++) {
						nomeG="MRtabs"+i;
						grid[nomeG] = new DCS.GridPraticheMaxirata({
										IdCategoriaMaxirata:arr[i].IdCategoriaMaxirata,
										task: "maxirata",
										title:arr[i].CategoriaMaxirata,
										titlePanel: 'Lista pratiche in '+arr[i].CategoriaMaxirata,
										stateful: true,
										stateId:"Maxrat"+arr[i].CodMaxirata
										});
						//idTabs.push(arr[i].IdCategoria);
						listP.push(grid[nomeG]);
					}
					Ext.getCmp('tabMr').add(listP);
					DCS.hideMask();
					Ext.getCmp('tabMr').setActiveTab(0);
				},
