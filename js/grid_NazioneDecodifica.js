...
DCS.GridNazioneTab = Ext.extend(Ext.grid.GridPanel, {
	pagesize: PAGESIZE,
	titlePanel: '',
	btnMenuAzioni: null,
	task: '',
	...
	initComponent : function() { 
		...
		var newRecord = function(btn, pressed)
		{
			...
			myMask.show();
			DCS.showDetailNAZ.create('Nuova voce nazione',null,gstore);
			myMask.hide();
	    };
	    ...
	    var delRecord = function(btn, pressed)
	    {
	    	...
	    	if(Arr.length>0){
		    	for(var k=0;k<Arr.length;k++){
		    		if(Arr[k].get('TitoloNazione')==null || Arr[k].get('TitoloNazione')=='')
		    			confString += '<br />	- *Titolo assente*';
		    		else
		    			confString += '<br />	-'+Arr[k].get('TitoloNazione');
		    		vectString = vectString + '|' + Arr[k].get('SiglaNazione');
		    	}
		    	Ext.MessageBox.alert('Conferma', "Si desidera eliminare: "+confString+" ?",function(btn, text){
		    	    if (btn == 'ok'){
		    	    	Ext.Ajax.request({
					        ...
					        method: 'POST',
					        params: {task: 'delete',vect: vectString, tipoDec:'nazioneDec'},
					        success: function(obj) {
					            var resp = obj.responseText;
					            console.log("res "+resp);
					            if (resp == '' && vectString!='') {
					                Ext.MessageBox.alert('Esito', 'Le nazioni selezionate sono stati eliminate dall\'elenco.');
					                gstore.reload();
					            } else {
					            	if(resp!=''){
		...
		var fields = [{name: 'SiglaNazione', type: 'string'},
							{name: 'TitoloNazione'},
							{name: 'LastUser'},
							{name: 'LastUpd', type:'date', dateFormat: 'Y-m-d H:i:s'}];

    	var columns = [selM,
    		        	{dataIndex:'TitoloNazione',	width:130,	header:'Nazione', hideable: false,filterable:true,groupable:false,sortable:true},
    		        	{dataIndex:'SiglaNazione',width:50, header:'Sigla',align:'center',hidden: false, hideable: false, filterable:true,groupable:false,sortable:true},    		        	
		...
		var gstore = new Ext.data.GroupingStore({
			...
			baseParams:{task: this.task, group: this.groupOn, tipoDec:'nazioneDec'},
			remoteSort: true,
...
//					}
					...
					myMask.show();
					var titolo="";
					if(rec.get('TitoloNazione')!=null)
						titolo = "Modifica nazione '"+rec.get('TitoloNazione')+"'";
					else 
						titolo = "Modifica nazione *Titolo assente*";
					DCS.showDetailNAZ.create(titolo,rec,gstore);
					myMask.hide();
				},
				scope: this
			}
		...
		DCS.GridNazioneTab.superclass.initComponent.call(this, arguments);
		this.activation();
		//this.store.load();
...
DCS.NazioniDec = function(){

	return {
		create: function(){
			var subtitle = '<span class="subtit"><br>Tabella delle sigle usate per identificare le nazioni, nelle tabelle dei Clienti e dei Recapiti</span>';

			var gridNazioni = new DCS.GridNazioneTab({
				titlePanel: 'Lista Nazioni'+subtitle,
...
DCS.dNazPanel = Ext.extend(Ext.Panel, {
	recordMod:null,
	titoloProc:'',
	Wmain:'',
	invisibileLeg:true,
...
//									width: 30,
				  ...
				  handler: function() {
	    		 		if (formAggregato.getForm().isValid()){
							...
							formAggregato.getForm().submit({
								...
								params: {task:"saveAgg", tipoDec:'nazioneDec'},
		...
		DCS.dNazPanel.superclass.initComponent.call(this);
		//editing
		if(this.recordMod!=null){
			Ext.getCmp(idDin+'sigla').setValue(this.recordMod.get('SiglaNazione'));
			Ext.getCmp(idDin+'nome').setValue(replace_Tospecial_chars(this.recordMod.get('TitoloNazione')));
			Ext.getCmp(idDin+'flagEditing').setValue(this.recordMod.get('SiglaNazione'));
			Ext.getCmp(idDin+'nome').fireEvent('change',Ext.getCmp(idDin+'nome'),this.recordMod.get('TitoloNazione'),'');
		}else{
			Ext.getCmp(idDin+'flagEditing').setValue(null);
		}
	}	// fine initcomponent
});
Ext.reg('DCS_DettaglioNationPanel', DCS.dNazPanel);
	
DCS.showDetailNAZ= function(titolo,rec,store)
{
	return {
		create: function(titolo,rec,store){
			...
			wind = new Ext.Window({
				...
				resizable:false,
				items: [{
					xtype: 'DCS_DettaglioNationPanel',
					titoloProc:'',
					recordMod:rec,
