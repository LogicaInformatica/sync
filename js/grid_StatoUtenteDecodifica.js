...
DCS.GridStUtTab = Ext.extend(Ext.grid.GridPanel, {
	pagesize: PAGESIZE,
	titlePanel: '',
	btnMenuAzioni: null,
	task: '',
	...
	initComponent : function() { 
		...
		var newRecord = function(btn, pressed)
		{
			...
			myMask.show();
			DCS.showDetailSUT.create('Nuovo stato',null,gstore);
			myMask.hide();
	    };
	    ...
	    var delRecord = function(btn, pressed)
	    {
	    	...
	    	if(Arr.length>0){
		    	for(var k=0;k<Arr.length;k++){
		    		if(Arr[k].get('TitoloStatoUtente')==null || Arr[k].get('TitoloStatoUtente')=='')
		    			confString += '<br />	- *Titolo assente*';
		    		else
		    			confString += '<br />	-'+Arr[k].get('TitoloStatoUtente');
		    		vectString = vectString + '|' + Arr[k].get('IdStatoUtente');
		    	}
		    	Ext.MessageBox.alert('Conferma', "Si desidera eliminare: "+confString+" ?",function(btn, text){
		    	    if (btn == 'ok'){
		    	    	Ext.Ajax.request({
					        ...
					        method: 'POST',
					        params: {task: 'delete',vect: vectString, tipoDec:'userState'},
		...
		var fields = [{name: 'IdStatoUtente', type: 'int'},
							{name: 'TitoloStatoUtente'},
							{name: 'CodStatoUtente', type: 'string'},
							{name: 'LastUser'},
							{name: 'LastUpd', type:'date', dateFormat: 'Y-m-d H:i:s'}];

    	var columns = [selM,
    	               	{dataIndex:'IdStatoUtente',width:10, header:'IdSU',hidden: true, hideable: false,filterable:true,groupable:false,sortable:false},
    		        	{dataIndex:'TitoloStatoUtente',	width:130,	header:'Stato utente', hideable: false,filterable:true,groupable:false,sortable:true},
    		        	{dataIndex:'CodStatoUtente',width:50, header:'Codice',align:'center',hidden: false, hideable: false, filterable:true,groupable:false,sortable:true},    		        	
		...
		var gstore = new Ext.data.GroupingStore({
			...
			baseParams:{task: this.task, group: this.groupOn, sort:this.sortfield, tipoDec:'userState'},
			remoteSort: true,
...
//					}
					...
					myMask.show();
					var titolo="";
					if(rec.get('TitoloStatoUtente')!=null)
						titolo = "Modifica stato '"+rec.get('TitoloStatoUtente')+"'";
					else 
						titolo = "Modifica stato *Titolo assente*";
					DCS.showDetailSUT.create(titolo,rec,gstore);
					myMask.hide();
				},
				scope: this
			}
		...
		DCS.GridStUtTab.superclass.initComponent.call(this, arguments);
		this.activation();
		//this.store.load();
...
DCS.StatoUtenza = function(){

	return {
		create: function(){
			var subtitle = '<span class="subtit"><br>Gli utenti in stato "operativo" sono gli unici a poter accedere al sistema; tutti gli altri stati sono "non operativi":'
				+'<br>se ne possono eventualmente definire di nuovi per distinguere i motivi di "non operativit&agrave;".</span>'

			var gridStatoUt = new DCS.GridStUtTab({
				titlePanel: 'Lista stati degli utenti'+subtitle,
				flex: 1,
				task: "readMainGrid"
			});
...
DCS.dUtStPanel = Ext.extend(Ext.Panel, {
	recordMod:null,
	titoloProc:'',
	Wmain:'',
	invisibileLeg:true,
...
//									width: 30,
				  ...
				  handler: function() {
	    		 		if (formAggregato.getForm().isValid()){
							...
							formAggregato.getForm().submit({
								...
								params: {task:"saveAgg", tipoDec:'userState'},
		...
		DCS.dUtStPanel.superclass.initComponent.call(this);
		//editing
		if(this.recordMod!=null){
			Ext.getCmp(idDin+'id').setValue(this.recordMod.get('IdStatoUtente'));
			Ext.getCmp(idDin+'nome').setValue(replace_Tospecial_chars(this.recordMod.get('TitoloStatoUtente')));
			Ext.getCmp(idDin+'codice').setValue(this.recordMod.get('CodStatoUtente'));
			Ext.getCmp(idDin+'nome').fireEvent('change',Ext.getCmp(idDin+'nome'),this.recordMod.get('TitoloStatoUtente'),'');
		}
	}	// fine initcomponent
});
Ext.reg('DCS_DettaglioStatoUtPanel', DCS.dUtStPanel);
	
DCS.showDetailSUT= function(titolo,rec,store)
{
	return {
		create: function(titolo,rec,store){
			...
			wind = new Ext.Window({
				...
				resizable:false,
				items: [{
					xtype: 'DCS_DettaglioStatoUtPanel',
					titoloProc:'',
					recordMod:rec,
