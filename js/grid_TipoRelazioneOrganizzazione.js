...
DCS.GridTRelazioneTab = Ext.extend(Ext.grid.GridPanel, {
	pagesize: PAGESIZE,
	titlePanel: '',
	btnMenuAzioni: null,
	task: '',
	...
	initComponent : function() { 
		...
		var newRecord = function(btn, pressed)
		{
			...
			myMask.show();
			DCS.showDetailTREL.create('Nuovo tipo di relazione',null,gstore);
			myMask.hide();
	    };
	    ...
	    var delRecord = function(btn, pressed)
	    {
	    	...
	    	if(Arr.length>0){
		    	for(var k=0;k<Arr.length;k++){
		    		if(Arr[k].get('TitoloTipoRelazione')==null || Arr[k].get('TitoloTipoRelazione')=='')
		    			confString += '<br />	- *Titolo assente*';
		    		else
		    			confString += '<br />	-'+Arr[k].get('TitoloTipoRelazione');
		    		vectString = vectString + '|' + Arr[k].get('IdTipoRelazione');
		    	}
		    	Ext.MessageBox.alert('Conferma', "Si desidera eliminare: "+confString+" ?",function(btn, text){
		    	    if (btn == 'ok'){
		    	    	Ext.Ajax.request({
					        ...
					        method: 'POST',
					        params: {task: 'delete',vect: vectString, tipoOrg:'trelazioneO'},
		...
		var fields = [{name: 'IdTipoRelazione', type: 'int'},
		              		{name: 'CodTipoRelazione'},
							{name: 'TitoloTipoRelazione'},
							{name: 'Ordine', type: 'int'},
							{name: 'LastUser'},
							{name: 'LastUpd', type:'date', dateFormat: 'Y-m-d H:i:s'}];

    	var columns = [selM,
    	               	{dataIndex:'IdTipoRelazione',width:10, header:'IdTrel',hidden: true, hideable: false,filterable:true,groupable:false,sortable:false},
    	               	{dataIndex:'Ordine',width:10, header:'order',hidden: true, hideable: false,filterable:true,groupable:false,sortable:false},
    	               	{dataIndex:'TitoloTipoRelazione',	width:100,	header:'Relazione', hideable: false,filterable:true,groupable:false,sortable:true},
    	               	{dataIndex:'CodTipoRelazione',width:50, header:'Codice',align:'center',hidden: false, hideable: false, filterable:true,groupable:false,sortable:true},
		...
		var gstore = new Ext.data.GroupingStore({
			...
			baseParams:{task: this.task, group: this.groupOn, tipoOrg:'trelazioneO'},
			remoteSort: true,
...
//					}
					...
					myMask.show();
					var titolo="";
					if(rec.get('TitoloTipoRelazione')!=null)
						titolo = "Modifica tipo di relazione '"+rec.get('TitoloTipoRelazione')+"'";
					else 
						titolo = "Modifica tipo di relazione *Titolo assente*";
					DCS.showDetailTREL.create(titolo,rec,gstore);
					myMask.hide();
				},
				scope: this
			}
		...
		DCS.GridTRelazioneTab.superclass.initComponent.call(this, arguments);
		this.activation();
		//this.store.load();
...
DCS.TRelazioneOrg = function(){

	return {
		create: function(){
		var subtitle = '<span class="subtit"><br>I tipi di relazione definiscono come sono collegati due soggetti della tabella dei Clienti: non devono essere modificati,'
			+"<br>se non quando si introducono nuove tipologie nei flussi di acquisizione dei Clienti dal sistema centrale.</span>";

			var gridtRelazione = new DCS.GridTRelazioneTab({
				titlePanel: 'Lista tipi di relazione'+subtitle,
...
DCS.dTrelazPanel = Ext.extend(Ext.Panel, {
	recordMod:null,
	titoloProc:'',
	Wmain:'',
	invisibileLeg:true,
...
//									width: 30,
				  ...
				  handler: function() {
	    		 		if (formAggregato.getForm().isValid()){
							...
							formAggregato.getForm().submit({
								...
								params: {task:"saveAgg", tipoOrg:'trelazioneO'},
		...
		DCS.dTrelazPanel.superclass.initComponent.call(this);
		//editing
		if(this.recordMod!=null){
			Ext.getCmp(idDin+'id').setValue(this.recordMod.get('IdTipoRelazione'));
			Ext.getCmp(idDin+'nome').setValue(replace_Tospecial_chars(this.recordMod.get('TitoloTipoRelazione')));
			Ext.getCmp(idDin+'sigla').setValue(this.recordMod.get('CodTipoRelazione'));
			Ext.getCmp(idDin+'nome').fireEvent('change',Ext.getCmp(idDin+'nome'),this.recordMod.get('TitoloTipoRelazione'),'');
		}else{
			Ext.getCmp(idDin+'flagEditing').setValue(null);
		}			
	}	// fine initcomponent
});
Ext.reg('DCS_DettaglioTipoRelazionePanel', DCS.dTrelazPanel);
	
DCS.showDetailTREL= function(titolo,rec,store)
{
	return {
		create: function(titolo,rec,store){
			...
			wind = new Ext.Window({
				...
				modal: true,
				title: titolo,
				tools: [helpTool("TipoRelazione")],
				resizable:false,
				items: [{
					xtype: 'DCS_DettaglioTipoRelazionePanel',
					titoloProc:'',
					recordMod:rec,
