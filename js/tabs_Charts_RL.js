...
Ext.namespace('DCS.ChartsRl');

DCS.ChartsRl.RiscattoLeasing = Ext.extend(Ext.Panel, {
	titlePanel: '',
	task: '',
	itipo: 1,
	dsMese: null,
	...
	initComponent : function() {
		...
		var dsTipoDataRiscattoLeasing = new Ext.data.Store({
			...
			baseParams:{	//this parameter is passed for any HTTP request
				task: 'read',
				sql: "SELECT 0 as IdCategoriaRiscattoLeasing, '&nbsp;' as CategoriaRiscattoLeasing UNION SELECT IdCategoriaRiscattoLeasing,CategoriaRiscattoLeasing FROM categoriaRiscattoLeasing" 
			},
			/*2. specify the reader*/
			reader:  new Ext.data.JsonReader(
				{
					root: 'results',//name of the property that is container for an Array of row objects
					id: 'IdCategoriaRiscattoLeasing'//the property within each row object that provides an ID for the record (optional)
				},
				[{name: 'IdCategoriaRiscattoLeasing'},
				{name: 'CategoriaRiscattoLeasing'}]
			),
			autoLoad: true
		});//end dsTipo
		
		this.comboLotto = new Ext.form.ComboBox({
		    triggerAction: 'all',
		    mode: 'local',
			lazyInit: false, 
		    ...
		    store: new Ext.data.ArrayStore({
	        	fields: ['lotto','descLotto'],
	        	data: [['30','Riscatti scaduti 0-30'],['60','Riscatti scaduti 30-60'],['90','Riscatti scaduti 60-90'],['90+','Riscatti scaduti oltre 90']]
	    	}),
		    valueField: 'lotto',
		    displayField: 'descLotto',
			width: 150,
			value: '30',
			listeners: {
...
//						this.selectYear(this.comboData.getRawValue());
		...
		this.comboData = new Ext.form.ComboBox({
			...
			forceSelection: true,
			autoSelect: true,
		    store: dsTipoDataRiscattoLeasing,
		    allowBlank: true,
		    valueField: 'IdCategoriaRiscattoLeasing',
		    displayField: 'CategoriaRiscattoLeasing',
			width: 150,
			listeners: {
				select: function(combo, record, index) {
					if (combo.getRawValue() === "" || combo.getRawValue() === "&nbsp;") {
			...
			listeners: {
				...
				select: function(combo, record, index) {
					if (combo.getRawValue() === "" || combo.getRawValue() === "&nbsp;") {
						...
						var lotto = this.comboLotto.getRawValue();
						if (dataType!=='') {
						  //Ext.getCmp(this.task+'_title').update('<h1>'+dataType+' - '+ record.data.FY + ' - ' + record.data.mese  + 
						  //		'<br><font size=2>'+ this.titlePanel+'</font></h1>');
						  Ext.getCmp(this.task+'_title').update('<h1>'+dataType+' - Anno '+ this.comboAnni.getValue() + ' - ' + lotto + ' - ' + record.data.mese  + '</h1>');			
						} else {
							//Ext.getCmp(this.task+'_title').update('<h1>' + record.data.FY + ' - ' + record.data.mese  + 
							//	'<br><font size=2>'+ this.titlePanel+'</font></h1>');
							Ext.getCmp(this.task+'_title').update('<h1> Anno '+ this.comboAnni.getValue() + ' - ' + lotto + ' - ' + record.data.mese + '</h1>');	
						}
				        ...
				        Ext.Ajax.request({
					        url: 'server/charts/riscattoleasing.php',
					        method: 'GET',
					        params: {type: 'stack', mese: record.data.num, task: this.task, data: dataType, lotto: this.comboLotto.getValue()},
					        success: function(obj) {
	                            ...
	                            if(arrRes.length>0) {
	                            	for (i = 0; i < arrRes.length; i++) {
						        		categoriaRiscattoLeasing = arrRes[i].CategoriaRiscattoLeasing;
						        		var ind = strDataSetNum.indexOf(categoriaRiscattoLeasing);
										if (ind<0) {
											var newDataset = {
										        label: arrRes[i].CategoriaRiscattoLeasing,
										        fill: false,
											...
											strDataSetNum.push(categoriaRiscattoLeasing);
											ind = strDataSetNum.indexOf(categoriaRiscattoLeasing);
										}
										this.gc.data.datasets[ind].data.push(arrRes[i].NumCategoriaRiscattoLeasing);	
									}
						        }
					        	this.gc.update();
							},	scope: this});  	
				        ...
				        Ext.Ajax.request({
					        ...
					        success: function(obj) {
	                            ...
	                            if(arrRes.length>0) {
	                            	for (i = 0; i < arrRes.length; i++) {
						        		categoriaRiscattoLeasing = arrRes[i].CategoriaRiscattoLeasing;
						        		var ind = strDataSetImporto.indexOf(categoriaRiscattoLeasing);
										if (ind<0) {
											var newDataset = {
										        label: arrRes[i].CategoriaRiscattoLeasing,
										        fill: false,
											...
											strDataSetImporto.push(categoriaRiscattoLeasing);
											ind = strDataSetImporto.indexOf(categoriaRiscattoLeasing);
										}
					...
					if (this.comboTipo.getValue()!=='Storico') {
					  ...
					  this.comboTipo.fireEvent('select',this.comboTipo,null,1);	
						...
						if (dataType!=='') {
						  ...
						  Ext.getCmp(this.task+'_title').update('<h1>'+dataType+' - Anno '+record.data.num + ' - ' + lotto +'</h1>');	
						} else {
							//Ext.getCmp(this.task+'_title').update('<h1> Fiscal Year '+record.data.num +'</h1>');
							Ext.getCmp(this.task+'_title').update('<h1> Anno '+record.data.num + ' - ' + lotto +'</h1>');
						}
				        ...
				        Ext.Ajax.request({
					        url: 'server/charts/riscattoleasingStory.php',
					        method: 'GET',
					        params: {type: 'stack', id:this.id, anno: record.data.num, task: this.task, data: dataType, lotto: this.comboLotto.getValue()},
					        success: function(obj) {
	                            ...
	                            if(arrRes.length>0) {
	                            	for (i = 0; i < arrRes.length; i++) {
						        		categoriaRiscattoLeasing = arrRes[i].CategoriaRiscattoLeasing;
						        		var ind = strDataSetNumStory.indexOf(categoriaRiscattoLeasing);
										if (ind<0) {
											var newDataset = {
										        label: arrRes[i].CategoriaRiscattoLeasing,
										        fill: false,
											...
											strDataSetNumStory.push(categoriaRiscattoLeasing);
											ind = strDataSetNumStory.indexOf(categoriaRiscattoLeasing);
										}
									    ...
									    this.gcStory.data.datasets[ind].data[parseInt(mese)-1] = arrRes[i].NumCategoriaRiscattoLeasing;	
									}
						        }
					        	this.gcStory.update();
					        },	scope: this});
					        ...
					        Ext.Ajax.request({
						        ...
						        success: function(obj) {
			                        ...
			                        if(arrRes.length>0) {
			                        	for (i = 0; i < arrRes.length; i++) {
							        		categoriaRiscattoLeasing = arrRes[i].CategoriaRiscattoLeasing;
							        		var ind = strDataSetImportoStory.indexOf(categoriaRiscattoLeasing);
											if (ind<0) {
												var newDataset = {
											        label: arrRes[i].CategoriaRiscattoLeasing,
											        fill: false,
												...
												strDataSetImportoStory.push(categoriaRiscattoLeasing);
												ind = strDataSetImportoStory.indexOf(categoriaRiscattoLeasing);
											}
											//inserisco i valori dell'agenzia al mese corrispondente
										    data = arrRes[i].Mese;
										    anno = data.substr(0,4);
		...
		Ext.apply(this,{
			...
			items: [{
				...
				items:[{
					...
					flex: 1
				},this.comboAnni,this.comboMese,this.comboTipo,this.comboData,this.comboLotto]
			},{
				xtype:'panel',
				layout: 'column',
				id:this.task+'_pnl',
...
//	                '->', {type:'button', text:'Grafico', icon:'images/3dgraph.png', handler:this.changeType, scope:this},
		...
		DCS.ChartsRl.RiscattoLeasing.superclass.initComponent.call(this, arguments);
	...
	changeType: function() {
		...
		this.itipo++; 
		if (this.itipo==DCS.ChartsRl.tipi.length) 
			this.itipo = 0;
		//g = g.clone( { swfUrl : 'FusionCharts/'+DCS.ChartsRl.tipi[this.itipo]+'.swf' } );
...
//--------------------------------------------------------------------
DCS.ChartsRl.presentaMese = function(idGrafico,numMese)
{
	//aaaa
...
//-----------------------------------------------------------
// Griglia con i dati statistici per categoria RiscattoLeasing
//-----------------------------------------------------------
DCS.ChartsRl.Statistiche = Ext.extend(Ext.grid.GridPanel, {
//	width:380,
    gstore: null,
	pagesize: 0,
	titlePanel: '',
	...
	initComponent : function() {
		var fields = [{name: 'IdCategoriaRiscattoLeasing'},
	                  {name: 'TotaleImportoInsoluto',type:'float'},
	                  {name: 'NumCategoriaRiscattoLeasing',type:'int'},
	                  {name: 'CategoriaRiscattoLeasing'},
	                  {name: 'Lotto'},
	                  {name: 'descrizioneLotto'}
	                 ];

	 	var columns = [{dataIndex:'CategoriaRiscattoLeasing',width:200, header:'Categoria Riscatti scaduti',sortable:true,align:'right'},
	   	               {dataIndex:'TotaleImportoInsoluto',width:200, header:'TOTALE Importo Insoluto',sortable:true,align:'right',css:'background-color:aquamarine;font-weight:bold;',xtype:'numbercolumn',format:'0.000,00/i'},
	   	               {dataIndex:'NumCategoriaRiscattoLeasing',width:100, header:'TOTALE N.',sortable:true,align:'right'},
	   	               {dataIndex:'descrizioneLotto',width:200, header:'Lotto',sortable:true,align:'right'}
	   	              ];
...
//			autoExpandColumn: true,
		...
		var tbarItems = [
			                ...
			                '-', helpButton("GraficiRiscattoLeasing"),' '
						];
		...
		DCS.ChartsRl.Statistiche.superclass.initComponent.call(this, arguments);
	}
});
...
//-----------------------------------------------------------
DCS.ChartsRl.StatRiscattoLeasing = Ext.extend(Ext.Panel, {
	titlePanel: '',
	task: '',
	itipo: 4,
	dsMese: null,
	 ...
	 */	
		...
		this.comboMese = new Ext.form.ComboBox({
			...
			listeners: {
				...
				select: function(combo, record, index) {
					...
					this.grid.titlePanel = "RiscattoLeasing "+record.data.mese;
		...
		Ext.apply(this,{
			...
			items: [{
				...
				xtype:'panel',
				layout: 'column',
				id:this.task+'_pageRl',
				items: [{
					xtype:'panel',
					columnWidth: .7,
					id:this.task+'_tableRl',
                    height:450,
					layout:'fit',
					items: [this.grid]
...
//	                '->', {type:'button', text:'Grafico', icon:'images/3dgraph.png', handler:this.changeType, scope:this},
		...
		DCS.ChartsRl.StatRiscattoLeasing.superclass.initComponent.call(this, arguments);
...
DCS.ChartsRl.TabsMRRL = function(){
	
	return {
		create_TFSI_RL: function(){
			items = Array();
			//if (CONTEXT.CAN_GRAPH_ALL)
				items.push(new DCS.ChartsRl.RiscattoLeasing({
					titlePanel: 'Riscatti scaduti',
					title: 'Riscatti scaduti',
					task: 'RiscattoLeasing',id: 'graphRiscattoLeasing'
					}));
					
			var targetGrid = new DCS.ChartsRl.Statistiche({
				titlePanel: 'Sintesi del periodo',
				stateful: true, gruppo: 1
				});
			
			items.push(new DCS.ChartsRl.StatRiscattoLeasing({
				titlePanel: 'Sintesi mensile Riscatti scaduti',
				title: 'Sintesi mensile Riscatti scaduti',
