...
DCS.GridPraticheStatiLegali = Ext.extend(DCS.GridPratiche, {
	IdCategoria:'',
	...
	initComponent : function() {
		...
		Ext.apply(this,{
	       ...
	       this.on('render',function(){
				...
				toolBar.insert(8,{
					   ...
					   text: 'Esporta tutto',
					   tooltip: 'Esporta su excel i dati relativi a tutti gli stati legali',
		...
		DCS.GridPraticheStatiLegali.superclass.initComponent.call(this,arguments);
	}
});
...
// Tabpanel 
//-----------------------------------------
DCS.PraticheStatiLegali = function() {
	//var idTabs;
	...
	return {
		...
		create: function(){
			...
			Ext.Ajax.request({
				...
				params: {
					task: 'read',
					sql: "SELECT IdStatoLegale,CodStatoLegale,TitoloStatoLegale,1 AS ordine FROM statolegale"
						 +" UNION ALL SELECT 0,'NUL','Non specificato',0 as ordine order by ordine,TitoloStatoLegale"
				},
				method: 'POST',
				autoload: true,
				success: function(result, request){
					...
					for(i=0;i<resp;i++){
						nomeG = "gridSle"+i; 
						tp.add(new DCS.GridPraticheStatiLegali({
							stateId: 'PraSle',
							stateful: true,
							titlePanel: 'Lista pratiche in stato: '+arr[i]['TitoloStatoLegale'],
							title: arr[i]['TitoloStatoLegale'],
							task: 'statolegale',
							sqlExtraCondition : arr[i]['IdStatoLegale']==0 ? "IdStatoLegale IS NULL": ("IdStatoLegale="+arr[i]['IdStatoLegale'])
						 })
						);
					}
					DCS.hideMask();
