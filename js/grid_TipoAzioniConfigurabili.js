...
DCS.GridTAzioTab = Ext.extend(Ext.grid.GridPanel, {
	pagesize: PAGESIZE,
	titlePanel: '',
	btnMenuAzioni: null,
	task: '',
	...
	initComponent : function() { 
		...
		var newRecord = function(btn, pressed)
		{
			...
			myMask.show();
			DCS.showDetailTAZIONI.create('Nuovo tipo',null,gstore);
			myMask.hide();
	    };
	    ...
	    var delRecord = function(btn, pressed)
	    {
	    	...
	    	if(Arr.length>0){
		    	for(var k=0;k<Arr.length;k++){
		    		if(Arr[k].get('TitoloTipoAzione')==null || Arr[k].get('TitoloTipoAzione')=='')
		    			confString += '<br />	- *Titolo assente*';
		    		else
		    			confString += '<br />	-'+Arr[k].get('TitoloTipoAzione');
		    		vectString = vectString + '|' + Arr[k].get('IdTipoAzione');
		    	}
		    	Ext.MessageBox.alert('Conferma', "Si desidera eliminare: "+confString+" ?",function(btn, text){
		    	    if (btn == 'ok'){
		    	    	Ext.Ajax.request({
					        ...
					        method: 'POST',
					        params: {task: 'delete',vect: vectString, tipoConf:'azioneConf'},
					        success: function(obj) {
					            var resp = obj.responseText;
					            console.log("res "+resp);
					            if (resp == '' && vectString!='') {
					                Ext.MessageBox.alert('Esito', 'I tipi di azioni selezionati sono stati eliminati');
					                gstore.reload();
					            } else {
					            	if(resp!=''){
		...
		var fields = [{name: 'IdTipoAzione', type: 'int'},
							{name: 'TitoloTipoAzione'},
							{name: 'CodTipoAzione', type: 'string'},
							{name: 'Ordine'},
							{name: 'LastUser'},
							{name: 'LastUpd', type:'date', dateFormat: 'Y-m-d H:i:s'}];

    	var columns = [selM,
    	               	{dataIndex:'IdTipoAzione',width:10, header:'IdTAz',hidden: true, hideable: false,filterable:true,groupable:false,sortable:false},
    		        	{dataIndex:'TitoloTipoAzione',	width:130,	header:'Tipo di Azione', hideable: false,filterable:true,groupable:false,sortable:true},
    		        	{dataIndex:'CodTipoAzione',width:50, header:'Codice',align:'center',hidden: false, hideable: false, filterable:true,groupable:false,sortable:true},    		        	
    		        	{dataIndex:'Ordine', width:50, header:'Ordine',align:'center',hidden: false, hideable: true, filterable:true,groupable:false,sortable:true},
		...
		var gstore = new Ext.data.GroupingStore({
			...
			baseParams:{task: this.task, group: this.groupOn, sort:this.sortfield, tipoConf:'azioneConf'},
			remoteSort: true,
...
//					}
					...
					myMask.show();
					var titolo="";
					if(rec.get('TitoloTipoAzione')!=null)
						titolo = "Modifica tipo '"+rec.get('TitoloTipoAzione')+"'";
					else 
						titolo = "Modifica tipo *Titolo assente*";
					DCS.showDetailTAZIONI.create(titolo,rec,gstore);
					myMask.hide();
				},
				scope: this
			}
		...
		DCS.GridTAzioTab.superclass.initComponent.call(this, arguments);
		this.activation();
		//this.store.load();
...
DCS.TipoAzioni = function(){

	return {
		create: function(){
			var subtitle = '<span class="subtit"><br>I tipi di azioni costituiscono le voci di primo livello del men&ugrave; a discesa "Azioni" (sulle liste).'
				+'<br>Sono liberamente modificabili con alcune eccezioni: "Crea nota" e "Crea messaggio" sono dedicati alle due azioni corrispondenti;'
				+'<br>mentre "Procedure speciali" &egrave; la voce sotto la quale compaiono tutti i workflow.</span>';

			var gridTAzioni = new DCS.GridTAzioTab({
				titlePanel: 'Lista tipi di azioni'+subtitle,
...
DCS.dTAzioniPanel = Ext.extend(Ext.Panel, {
	recordMod:null,
	titoloProc:'',
	Wmain:'',
	store:null,
...
//									width: 30,
				  ...
				  handler: function() {
	    		 		if (formAggregato.getForm().isValid()){
							...
							formAggregato.getForm().submit({
								...
								params: {task:"saveAgg", tipoConf:'azioneConf'},
		...
		DCS.dTAzioniPanel.superclass.initComponent.call(this);
		//editing
		if(this.recordMod!=null){
			Ext.getCmp(idDin+'id').setValue(this.recordMod.get('IdTipoAzione'));
			Ext.getCmp(idDin+'nome').setValue(replace_Tospecial_chars(this.recordMod.get('TitoloTipoAzione')));
			Ext.getCmp(idDin+'codice').setValue(this.recordMod.get('CodTipoAzione'));
			Ext.getCmp(idDin+'nome').fireEvent('change',Ext.getCmp(idDin+'nome'),this.recordMod.get('TitoloTipoAzione'),'');
		}
	}	// fine initcomponent
});
Ext.reg('DCS_DettaglioTazioniPanel', DCS.dTAzioniPanel);
	
DCS.showDetailTAZIONI= function()
{
	return {
		create: function(titolo,rec,store){
			wind = new Ext.Window({
				...
				modal: true,
				title: titolo,
				tools: [helpTool("Azione")],
				resizable:false,
				items: [{
					xtype: 'DCS_DettaglioTazioniPanel',
					titoloProc:'',
					recordMod:rec,
					store:store
					}]
