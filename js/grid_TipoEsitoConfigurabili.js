...
DCS.GridTEsitoTab = Ext.extend(Ext.grid.GridPanel, {
	pagesize: PAGESIZE,
	titlePanel: '',
	btnMenuAzioni: null,
	task: '',
	...
	initComponent : function() { 
		...
		var newRecord = function(btn, pressed)
		{
			...
			myMask.show();
			DCS.showDetailTESIT.create('Nuova tipo esito',null,gstore);
			myMask.hide();
	    };
	    ...
	    var delRecord = function(btn, pressed)
	    {
	    	...
	    	if(Arr.length>0){
		    	for(var k=0;k<Arr.length;k++){
		    		if(Arr[k].get('TitoloTipoEsito')==null || Arr[k].get('TitoloTipoEsito')=='')
		    			confString += '<br />	- *Titolo assente*';
		    		else
		    			confString += '<br />	-'+Arr[k].get('TitoloTipoEsito');
		    		vectString = vectString + '|' + Arr[k].get('IdTipoEsito');
		    	}
		    	Ext.MessageBox.alert('Conferma', "Si desidera eliminare: "+confString+" ?",function(btn, text){
		    	    if (btn == 'ok'){
		    	    	Ext.Ajax.request({
					        ...
					        method: 'POST',
					        params: {task: 'delete',vect: vectString, tipoConf:'tesitoConf'},
		...
		var fields = [{name: 'IdTipoEsito', type: 'int'},
		              		{name: 'CodTipoEsito'},
							{name: 'TitoloTipoEsito'},
							{name: 'Negativo'},
							{name: 'LastUser'},
							{name: 'LastUpd', type:'date', dateFormat: 'Y-m-d H:i:s'}];

    	var columns = [selM,
    	               	{dataIndex:'IdTipoEsito',width:10, header:'IdTEsito',hidden: true, hideable: false,filterable:true,groupable:false,sortable:false},
    	               	{dataIndex:'TitoloTipoEsito',	width:100,	header:'Tipo esito', hideable: false,filterable:true,groupable:false,sortable:true},
    	               	{dataIndex:'CodTipoEsito',width:50, header:'Codice',align:'center',hidden: false, hideable: false, filterable:true,groupable:false,sortable:true},
    	               	{dataIndex:'Negativo',width:16, exportable:false, renderer:DCS.render.spunta, header:'Negativo',align:'center', sizable:false, menuDisabled:true, hideable:false, groupable:false, sortable:false, hidden:false},
		...
		var gstore = new Ext.data.GroupingStore({
			...
			baseParams:{task: this.task, group: this.groupOn, tipoConf:'tesitoConf'},
			remoteSort: true,
...
//					}
					...
					myMask.show();
					var titolo="";
					if(rec.get('TitoloTipoEsito')!=null)
						titolo = "Modifica tipo esito '"+rec.get('TitoloTipoEsito')+"'";
					else 
						titolo = "Modifica tipo esito *Titolo assente*";
					DCS.showDetailTESIT.create(titolo,rec,gstore);
					myMask.hide();
				},
				scope: this
			}
		...
		DCS.GridTEsitoTab.superclass.initComponent.call(this, arguments);
		this.activation();
		//this.store.load();
...
DCS.TEsitoConf = function(){

	return {
		create: function(){
			var subtitle = '<span class="subtit"><br>I tipi di esito sono liberamente definibili per essere specificati nelle azioni di tipo "registrazione esito", ad es. la Telefonata al cliente.</span>';

			var gridtesito = new DCS.GridTEsitoTab({
				titlePanel: 'Lista tipi di esito'+subtitle,
...
DCS.dTEsitoPanel = Ext.extend(Ext.Panel, {
	recordMod:null,
	titoloProc:'',
	Wmain:'',
	store:null,
...
//									width: 30,
				  ...
				  handler: function() {
	    		 		if (formAggregato.getForm().isValid()){
							...
							formAggregato.getForm().submit({
								...
								params: {task:"saveAgg", tipoConf:'tesitoConf'},
		...
		DCS.dTEsitoPanel.superclass.initComponent.call(this);
		//editing
		if(this.recordMod!=null){
			Ext.getCmp(idDin+'id').setValue(this.recordMod.get('IdTipoEsito'));
			Ext.getCmp(idDin+'nome').setValue(replace_Tospecial_chars(this.recordMod.get('TitoloTipoEsito')));
			Ext.getCmp(idDin+'sigla').setValue(this.recordMod.get('CodTipoEsito'));
			Ext.getCmp(idDin+'chk').setValue(this.recordMod.get('Negativo')=="Y"?true:false);
			Ext.getCmp(idDin+'nome').fireEvent('change',Ext.getCmp(idDin+'nome'),this.recordMod.get('TitoloTipoEsito'),'');
		}else{
			Ext.getCmp(idDin+'flagEditing').setValue(null);
		}			
	}	// fine initcomponent
});
Ext.reg('DCS_DettaglioTipoEsitoPanel', DCS.dTEsitoPanel);
	
DCS.showDetailTESIT= function()
{
	return {
		create: function(titolo,rec,store){
			wind = new Ext.Window({
				...
				modal: true,
				title: titolo,
				tools: [helpTool("TipoEsito")],
				resizable:false,
				items: [{
					xtype: 'DCS_DettaglioTipoEsitoPanel',
					titoloProc:'',
					recordMod:rec,
					store:store
					}]
