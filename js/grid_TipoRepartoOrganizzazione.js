...
DCS.GridTRepTab = Ext.extend(Ext.grid.GridPanel, {
	pagesize: PAGESIZE,
	titlePanel: '',
	btnMenuAzioni: null,
	task: '',
	...
	initComponent : function() { 
		...
		var newRecord = function(btn, pressed)
		{
			...
			myMask.show();
			DCS.showDetailTREP.create('Nuovo tipo di reparto',null,gstore);
			myMask.hide();
	    };
	    ...
	    var delRecord = function(btn, pressed)
	    {
	    	...
	    	if(Arr.length>0){
		    	for(var k=0;k<Arr.length;k++){
		    		if(Arr[k].get('TitoloTipoReparto')==null || Arr[k].get('TitoloTipoReparto')=='')
		    			confString += '<br />	- *Titolo assente*';
		    		else
		    			confString += '<br />	-'+Arr[k].get('TitoloTipoReparto');
		    		vectString = vectString + '|' + Arr[k].get('IdTipoReparto');
		    	}
		    	Ext.MessageBox.alert('Conferma', "Si desidera eliminare: "+confString+" ?",function(btn, text){
		    	    if (btn == 'ok'){
		    	    	Ext.Ajax.request({
					        ...
					        method: 'POST',
					        params: {task: 'delete',vect: vectString, tipoOrg:'trepartoO'},
		...
		var fields = [{name: 'IdTipoReparto', type: 'int'},
		              		{name: 'CodTipoReparto'},
    	...
    	var columns = [selM,
    	               	{dataIndex:'IdTipoReparto',width:10, header:'IdTr',hidden: true, hideable: false,filterable:true,groupable:false,sortable:false},
    	               	{dataIndex:'TitoloTipoReparto',	width:100,	header:'Reparto', hideable: false,filterable:true,groupable:false,sortable:true},
    	               	{dataIndex:'CodTipoReparto',width:50, header:'Codice',align:'center',hidden: false, hideable: false, filterable:true,groupable:false,sortable:true},
		...
		var gstore = new Ext.data.GroupingStore({
			...
			baseParams:{task: this.task, group: this.groupOn, tipoOrg:'trepartoO'},
			remoteSort: true,
...
//					}
					...
					myMask.show();
					var titolo="";
					if(rec.get('TitoloTipoReparto')!=null)
						titolo = "Modifica tipo reparto '"+rec.get('TitoloTipoReparto')+"'";
					else 
						titolo = "Modifica tipo reparto *Titolo assente*";
					DCS.showDetailTREP.create(titolo,rec,gstore);
					myMask.hide();
				},
				scope: this
			}
		...
		DCS.GridTRepTab.superclass.initComponent.call(this, arguments);
		this.activation();
		//this.store.load();
...
DCS.TRepartoOrg = function(){

	return {
		create: function(){
			var subtitle = '<span class="subtit"><br>I tipi di reparto costituiscono un insieme fisso predefinito, che distingue i vari tipi di enti interni ed esterni.'
				+'<br>Non dovrebbero mai essere modificati.</span>';

			var gridtRep = new DCS.GridTRepTab({
				titlePanel: 'Lista tipi di reparto'+subtitle,
...
DCS.dTrepPanel = Ext.extend(Ext.Panel, {
	recordMod:null,
	titoloProc:'',
	Wmain:'',
	invisibileLeg:true,
...
//									width: 30,
				  ...
				  handler: function() {
	    		 		if (formAggregato.getForm().isValid()){
							...
							formAggregato.getForm().submit({
								...
								params: {task:"saveAgg", tipoOrg:'trepartoO'},
		...
		DCS.dTrepPanel.superclass.initComponent.call(this);
		//editing
		if(this.recordMod!=null){
			Ext.getCmp(idDin+'id').setValue(this.recordMod.get('IdTipoReparto'));
			Ext.getCmp(idDin+'nome').setValue(replace_Tospecial_chars(this.recordMod.get('TitoloTipoReparto')));
			Ext.getCmp(idDin+'sigla').setValue(this.recordMod.get('CodTipoReparto'));
			Ext.getCmp(idDin+'nome').fireEvent('change',Ext.getCmp(idDin+'nome'),this.recordMod.get('TitoloTipoReparto'),'');
		}else{
			Ext.getCmp(idDin+'flagEditing').setValue(null);
		}			
	}	// fine initcomponent
});
Ext.reg('DCS_DettaglioTipoRepartoPanel', DCS.dTrepPanel);
	
DCS.showDetailTREP= function(titolo,rec,store)
{
	return {
		create: function(titolo,rec,store){
			...
			wind = new Ext.Window({
				...
				modal: true,
				title: titolo,
				tools: [helpTool("TipoReparto")],
				resizable:false,
				items: [{
					xtype: 'DCS_DettaglioTipoRepartoPanel',
					titoloProc:'',
					recordMod:rec,
