...
DCS.GridPagamentoTab = Ext.extend(Ext.grid.GridPanel, {
	pagesize: PAGESIZE,
	titlePanel: '',
	btnMenuAzioni: null,
	task: '',
	...
	initComponent : function() { 
		...
		var newRecord = function(btn, pressed)
		{
			...
			myMask.show();
			DCS.showDetailPAG.create('Nuovo tipo pagamento',null,gstore);
			myMask.hide();
	    };
	    ...
	    var delRecord = function(btn, pressed)
	    {
	    	...
	    	if(Arr.length>0){
		    	for(var k=0;k<Arr.length;k++){
		    		if(Arr[k].get('TitoloTipoPagamento')==null || Arr[k].get('TitoloTipoPagamento')=='')
		    			confString += '<br />	- *Titolo assente*';
		    		else
		    			confString += '<br />	-'+Arr[k].get('TitoloTipoPagamento');
		    		vectString = vectString + '|' + Arr[k].get('IdTipoPagamento');
		    	}
		    	Ext.MessageBox.alert('Conferma', "Si desidera eliminare: "+confString+" ?",function(btn, text){
		    	    if (btn == 'ok'){
		    	    	Ext.Ajax.request({
					        ...
					        method: 'POST',
					        params: {task: 'delete',vect: vectString, tipoDec:'pagamento'},
					        success: function(obj) {
					            var resp = obj.responseText;
					            console.log("res "+resp);
					            if (resp == '' && vectString!='') {
					                Ext.MessageBox.alert('Esito', 'I tipi pagametni selezionati sono state eliminati.');
					                gstore.reload();
					            } else {
					            	if(resp!=''){
		...
		var fields = [{name: 'IdTipoPagamento', type: 'int'},
							{name: 'TitoloTipoPagamento'},
							{name: 'CodTipoPagamentoLegacy', type: 'string'},
							{name: 'CodTipoPagamento', type: 'string'},
							{name: 'LastUser'},
							{name: 'LastUpd', type:'date', dateFormat: 'Y-m-d H:i:s'}];

    	var columns = [selM,
    	               	{dataIndex:'IdTipoPagamento',width:10, header:'IdTP',hidden: true, hideable: false,filterable:true,groupable:false,sortable:false},
    		        	{dataIndex:'TitoloTipoPagamento',	width:130,	header:'Pagamento', hideable: false,filterable:true,groupable:false,sortable:true},
    		        	{dataIndex:'CodTipoPagamentoLegacy',width:50, header:'Vecchio codice',align:'center',hidden: false, hideable: false, filterable:true,groupable:false,sortable:true},    		        	
    		        	{dataIndex:'CodTipoPagamento',	width:50,	header:'Nuovo codice',align:'center',hidden: false, hideable: false,filterable:true,groupable:false,sortable:true},
		...
		var gstore = new Ext.data.GroupingStore({
			...
			baseParams:{task: this.task, group: this.groupOn, tipoDec:'pagamento'},
			remoteSort: true,
...
//					}
					...
					myMask.show();
					var titolo="";
					if(rec.get('TitoloTipoPagamento')!=null)
						titolo = "Modifica tipo pagamento '"+rec.get('TitoloTipoPagamento')+"'";
					else 
						titolo = "Modifica tipo pagamento *Titolo assente*";
					DCS.showDetailPAG.create(titolo,rec,gstore);
					myMask.hide();
				},
				scope: this
			}
		...
		DCS.GridPagamentoTab.superclass.initComponent.call(this, arguments);
		this.activation();
		//this.store.load();
...
DCS.TipoPagamento = function(){

	return {
		create: function(){
			var subtitle = '<span class="subtit"><br>I codici di "tipo pagamento" distinguono in particolare i contratti con pagamento bancario da quelli con bollettino postale.'
				+'<br>Si noti che questo indicatore &egrave; uno dei parametri che si possono usare nelle Regole di classificazione.</span>';

			var gridPagamento = new DCS.GridPagamentoTab({
				titlePanel: 'Lista tipi di pagamento'+subtitle,
...
DCS.dPagPanel = Ext.extend(Ext.Panel, {
	recordMod:null,
	titoloProc:'',
	Wmain:'',
	invisibileLeg:true,
...
//									width: 30,
				  ...
				  handler: function() {
	    		 		if (formAggregato.getForm().isValid()){
							...
							formAggregato.getForm().submit({
								...
								params: {task:"saveAgg", tipoDec:'pagamento'},
		...
		DCS.dPagPanel.superclass.initComponent.call(this);
		//editing
		if(this.recordMod!=null){
			Ext.getCmp(idDin+'id').setValue(this.recordMod.get('IdTipoPagamento'));
			Ext.getCmp(idDin+'nome').setValue(replace_Tospecial_chars(this.recordMod.get('TitoloTipoPagamento')));
			Ext.getCmp(idDin+'codice').setValue(this.recordMod.get('CodTipoPagamento'));
			Ext.getCmp(idDin+'codiceL').setValue(this.recordMod.get('CodTipoPagamentoLegacy'));
			Ext.getCmp(idDin+'nome').fireEvent('change',Ext.getCmp(idDin+'nome'),this.recordMod.get('TitoloTipoPagamento'),'');
		}
	}	// fine initcomponent
});
Ext.reg('DCS_DettaglioPagamentoPanel', DCS.dPagPanel);
	
DCS.showDetailPAG= function(titolo,rec,store)
{
	return {
		create: function(titolo,rec,store){
			var invisibileLeg=true;
			var h=165;
			if(rec!=null && rec.get('CodTipoPagamentoLegacy')!='' && rec.get('CodTipoPagamentoLegacy')!=null)
			{
				invisibileLeg=false;
				h=190;
			}
			wind = new Ext.Window({
				...
				modal: true,
				title: titolo,
				tools: [helpTool("TipoPagamento")],
				resizable:false,
				items: [{
					xtype: 'DCS_DettaglioPagamentoPanel',
					titoloProc:'',
					recordMod:rec,
