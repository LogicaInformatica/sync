...
Ext.ux.form.ExtendedComboBox = Ext.extend(Ext.form.ComboBox, {
    ...
    initComponent: function() {
        ...
        // Funzione estratta dalla .expand per poterla chiamare alla onload dello store (GDF)
        var paintBottomToolbar = function ()
        {
        	if (me.extendedPageTb)
        		return;					// già presente (reload o simili)
                    ...
                    menu: [
                        {
                            ...
                            text: me.limitSelection <= 0 ? 'Tutti' : 'Tutti (fino a: ' + me.limitSelection + ')',
...
/*GDF                // only paint the bottom toolbar if:
        ...
        var onStoreLoadFunction = function(store, records) {
            ...
            paintBottomToolbar(); // aggiunto GDF
        };
...
//      if(select_fav) {
        if (record.get(this.checkField)) { ///  correzione di GDF (this.checkField = "Selected" è la checkbox di riga)
        	record.set(this.checkField,false); ///  correzione di GDF // entra qui quando è una deselezione
            ...
            this.setValue(this.getCheckedValue()); // GDF: altrimenti non aggiorna il value quando si deseleziona
