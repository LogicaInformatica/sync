...
create or replace view db_cnc_storico.v_praticheAzioniSpeciali as
select
...
from db_cnc_storico.azionespeciale as asp
left join azione a on asp.IdAzione = a.IdAzione
left join db_cnc_storico.contratto c on asp.IdContratto = c.IdContratto
...
left join db_cnc_storico.storiarecupero sr on sr.IdAzioneSpeciale = asp.IdAzioneSpeciale
left join utente u ON sr.IdSuper=u.IdUtente
left join db_cnc_storico.allegatoazionespeciale aasp on aasp.IdAzioneSpeciale = asp.IdAzioneSpeciale
left join db_cnc_storico.pianorientro pr on pr.IdContratto = asp.IdContratto
group by
